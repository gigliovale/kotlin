            ...
            functionCodegen.generateMethod(OtherOrigin(myClass, function), function, new FunctionGenerationStrategy() {
                ...
                private void pushCapturedFieldsOnStack(InstructionAdapter iv, MutableClosure closure) {
                    ...
                    for (Map.Entry<DeclarationDescriptor, EnclosedValueDescriptor> entry : closure.getCaptureVariables().entrySet()) {
                        DeclarationDescriptor capturedDeclarationDescriptor = entry.getKey();
                        EnclosedValueDescriptor capturedEnclosedValueDescriptor = entry.getValue();

                        Type sharedVarType = typeMapper.getSharedVarType(capturedDeclarationDescriptor);
                        if (sharedVarType == null) {
                            sharedVarType = typeMapper.mapType((VariableDescriptor) capturedDeclarationDescriptor);
                        }
                        capturedEnclosedValueDescriptor.getInnerValue().put(sharedVarType, iv);
                    }
                }
            });
