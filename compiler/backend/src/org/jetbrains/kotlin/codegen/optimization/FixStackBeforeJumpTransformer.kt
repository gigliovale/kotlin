...
class FixStackBeforeJumpTransformer : MethodTransformer() {
    public override fun transform(internalClassName: String, methodNode: MethodNode) {
            ...
            when {
                PseudoInsnOpcode.FIX_STACK_BEFORE_JUMP.isa(insnNode) -> {
                    ...
                    if (next.getOpcode() != Opcodes.GOTO)
                        throw AssertionError("Instruction after ${PseudoInsnOpcode.FIX_STACK_BEFORE_JUMP} should be GOTO")
                    jumpsToFix.add(next as JumpInsnNode)
                }
                PseudoInsnOpcode.FAKE_ALWAYS_TRUE_IFEQ.isa(insnNode) -> {
                    if (insnNode.getNext().getOpcode() != Opcodes.IFEQ)
                        throw AssertionError("Instruction after ${PseudoInsnOpcode.FAKE_ALWAYS_TRUE_IFEQ} should be IFEQ")
                    fakesToGotos.add(insnNode)
                }
                PseudoInsnOpcode.FAKE_ALWAYS_FALSE_IFEQ.isa(insnNode) -> {
                    if (insnNode.getNext().getOpcode() != Opcodes.IFEQ)
                        throw AssertionError("Instruction after ${PseudoInsnOpcode.FAKE_ALWAYS_FALSE_IFEQ} should be IFEQ")
        ...
        if (jumpsToFix.isNotEmpty()) {
            val analyzer = StackDepthAnalyzer(internalClassName, methodNode, LinkedHashSet<JumpInsnNode>(jumpsToFix))
            ...
            for (jumpNode in jumpsToFix) {
                ...
                if (actualStackSize != DEAD_CODE && expectedStackSize != DEAD_CODE) {
                    if (expectedStackSize > actualStackSize) {
                        throw AssertionError("Label at $labelIndex, jump at $jumpIndex: stack underflow: $expectedStackSize > $actualStackSize")
                    }
        ...
        for (marker in fakesToGotos) {
            actions.add({ replaceAlwaysTrueIfeqWithGoto(methodNode, marker) })
        }

        for (marker in fakesToRemove) {
            actions.add({ removeAlwaysFalseIfeq(methodNode, marker) })
        }
