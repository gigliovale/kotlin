...
public open class MethodAnalyzer<V : Value>(
    ...
    public val analyzable: Boolean = (method.access and (Opcodes.ACC_ABSTRACT or Opcodes.ACC_NATIVE)) == 0
    public val handlers: Array<MutableList<TryCatchBlockNode>?> = arrayOfNulls(nInsns)
    public val frames: Array<Frame<V>?> = if (analyzable) arrayOfNulls(nInsns) else arrayOf()

    private var queued: BooleanArray = BooleanArray(nInsns)
    private var queue: IntArray = IntArray(nInsns)
    ...
    private fun initAnalysisContext() {
        queued = BooleanArray(nInsns)
        queue = IntArray(nInsns)
        top = 0
    }
