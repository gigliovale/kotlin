...
import org.jetbrains.kotlin.codegen.optimization.RepeatUntilNoChangesMethodTransformer
...
class RedundantNullCheckMethodTransformer : RepeatUntilNoChangesMethodTransformer() {
    override fun doTransform(internalClassName: String, methodNode: MethodNode): Boolean =
            TransformerPass(internalClassName, methodNode).run()
    ...
    private class TransformerPass(val internalClassName: String, val methodNode: MethodNode) {
        ...
        private inner class NullabilityAssumptionsBuilder(val checkedReferenceTypes: Map<AbstractInsnNode, Type>) {
            ...
            private fun NullabilityAssumptions.injectAssumptionsForNullCheck(varIndex: Int, insn: JumpInsnNode, varType: Type) {
                ...
                methodNode.instructions.run {
                    ...
                    insert(insertAfterNull, rawSynthetics {
                    ...
                    insert(insertAfterNonNull, rawSynthetics {
                        add(TypeInsnNode(Opcodes.NEW, varType.internalName))
                        add(VarInsnNode(Opcodes.ASTORE, varIndex))
                        if (!jumpsIfNull) {
                            add(JumpInsnNode(Opcodes.GOTO, originalLabel))
                        }
            ...
            private fun NullabilityAssumptions.injectAssumptionsForParameterNotNullCheck(varIndex: Int, insn: AbstractInsnNode, varType: Type) {
                ...
                methodNode.instructions.insert(insn, rawSynthetics {
                    add(TypeInsnNode(Opcodes.NEW, varType.internalName))
                    add(VarInsnNode(Opcodes.ASTORE, varIndex))
                })
            }
            ...
            private fun NullabilityAssumptions.injectAssumptionsForInstanceOfCheck(varIndex: Int, insn: AbstractInsnNode, varType: Type) {
                ...
                methodNode.instructions.run {
                    insert(insertAfterNotNull, rawSynthetics {
                        add(TypeInsnNode(Opcodes.NEW, varType.internalName))
                        add(VarInsnNode(Opcodes.ASTORE, varIndex))
        ...
        inner class NullabilityAssumptions {
            ...
            inline fun rawSynthetics(builder: InsnList.() -> Unit) =
                    InsnList().apply {
                        builder()
                        asSequence().forEach { syntheticInstructions.add(it) }
                    }
