...
fun <T : Any> copyBean(bean: T, deepCopyWhenNeeded: Boolean = false) = copyFields(bean, bean.javaClass.newInstance(), deepCopyWhenNeeded)

fun <From : Any, To : From> mergeBeans(from: From, to: To, deepCopyWhenNeeded: Boolean = false): To {
    // TODO: rewrite when updated version of com.intellij.util.xmlb is available on TeamCity
    return copyFields(from, XmlSerializerUtil.createCopy(to), deepCopyWhenNeeded)
}
