...
class LanguageFeatureMessageRenderer @JvmOverloads constructor(
    ...
    override fun render(obj: Pair<LanguageFeature, LanguageVersionSettings>, renderingContext: RenderingContext): String {
        ...
        when (type) {
            Type.UNSUPPORTED -> if (since == null) {
                sb.append("experimental and should be enabled explicitly")
            }
            else if(since > settings.languageVersion) {
                sb.append("only available since ").append(since.versionString).append(" language version")
            }
            else if (feature.sinceApiVersion > settings.apiVersion) {
                sb.append("only available since ").append(feature.sinceApiVersion.versionString).append(" API version")
            }
            else {
                sb.append("disabled")
            }
