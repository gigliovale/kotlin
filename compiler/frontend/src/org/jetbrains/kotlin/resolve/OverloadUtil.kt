...
object OverloadUtil {
     ...
     */
    fun isOverloadable(a: CallableDescriptor, b: CallableDescriptor): OverloadCompatibilityInfo {
        ...
        when (overrideCompatibilityInfo.result) {
            OverridingUtil.OverrideCompatibilityInfo.Result.OVERRIDABLE, OverridingUtil.OverrideCompatibilityInfo.Result.CONFLICT -> return OverloadCompatibilityInfo.someError()
    ...
    private fun braceCount(a: CallableDescriptor): Int {
        if (a is PropertyDescriptor) {
            return 0
        }
        else if (a is SimpleFunctionDescriptor) {
            return 1
        }
        else if (a is ConstructorDescriptor) {
            return 1
        }
        else {
    ...
    class OverloadCompatibilityInfo(val isSuccess: Boolean, val message: String) {
        companion object {
            ...
            fun success(): OverloadCompatibilityInfo {
                return SUCCESS
            }

            fun someError(): OverloadCompatibilityInfo {
                return OverloadCompatibilityInfo(false, "XXX")
            }
        }
    }
