...
class CoroutineInferenceSupport(
    ...
    @set:Inject
    lateinit var genericCandidateResolver: GenericCandidateResolver
    ...
    fun analyzeCoroutine(
        ...
        // this trace should be not committed
    ...
    private fun isBadCall(resultingDescriptor: CallableDescriptor): Boolean {
        ...
        fun FunctionDescriptor.isHandleResult() = isOperator && name == OperatorNameConventions.COROUTINE_HANDLE_RESULT

        val returnType = resultingDescriptor.returnType ?: return true
        if (returnType.containsTypeTemplate()) return true

        if (resultingDescriptor !is FunctionDescriptor || resultingDescriptor.isSuspend || resultingDescriptor.isHandleResult()) return false
