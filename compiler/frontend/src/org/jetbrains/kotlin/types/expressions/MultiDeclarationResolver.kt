...
public class MultiDeclarationResolver(
    ...
    public fun defineLocalVariablesFromMultiDeclaration(
        ...
        for (entry in multiDeclaration.getEntries()) {
            val componentName = createComponentName(componentIndex)
            componentIndex++
            ...
            if (results.isSuccess()) {
                context.trace.record<JetMultiDeclarationEntry, ResolvedCall<FunctionDescriptor>>(BindingContext.COMPONENT_RESOLVED_CALL, entry, results.getResultingCall())
    ...
    private fun getExpectedTypeForComponent(context: ExpressionTypingContext, entry: JetMultiDeclarationEntry): JetType {
        val entryTypeRef = entry.getTypeReference()
