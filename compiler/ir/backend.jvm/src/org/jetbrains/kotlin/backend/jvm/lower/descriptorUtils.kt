...
fun FunctionDescriptor.toStatic(
    ...
    val oldExtensionReceiver = extensionReceiverParameter
    ...
    val valueParameters = listOf(dispatchReceiver, extensionReceiver).filterNotNull() +
                                                     valueParameters.map { it.copy(newFunction, it.name, it.index + 1) }

    newFunction.initialize(
            null, null, emptyList()/*TODO: type parameters*/,
            valueParameters, returnType, Modality.FINAL, visibility
    )
    return newFunction
}