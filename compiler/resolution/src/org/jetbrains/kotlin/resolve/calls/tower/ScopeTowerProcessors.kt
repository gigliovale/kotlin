...
class KnownResultProcessor<out C>(
    ...
    override fun process(data: TowerData)
            = if (data == TowerData.Empty) listOfNotNull(result.passIf { it.isNotEmpty() }) else emptyList()
}
...
internal abstract class AbstractSimpleScopeTowerProcessor<D : CallableDescriptor, C: Candidate<D>>(
    ...
    override fun process(data: TowerData): List<Collection<C>> = listOfNotNull(simpleProcess(data).passIf { it.isNotEmpty() })
}
