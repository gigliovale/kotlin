...
import org.jetbrains.kotlin.idea.stubindex.JetFileFacadeFqnIndex
...
public class JetShortNamesCache(private val project: Project) : PsiShortNamesCache() {
    ...
    override fun getAllClassNames(): Array<String> {
        ...
        classNames.addAll(JetFileFacadeFqnIndex.getAllFacadeShortNames(project))
    ...
    override fun getClassesByName(name: String, scope: GlobalSearchScope): Array<PsiClass> {
        ...
        // package classes can not be indexed, since they have no explicit declarations
        val fqNames = packageClasses.get(name) + JetFileFacadeFqnIndex.getInstance().getAllKeys(project).map { FqName(it) }.filter { it.shortName().asString() == name }
