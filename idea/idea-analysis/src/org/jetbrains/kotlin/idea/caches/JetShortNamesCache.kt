...
public class JetShortNamesCache(private val project: Project) : PsiShortNamesCache() {
    ...
    override fun getClassesByName(name: String, scope: GlobalSearchScope): Array<PsiClass> {
        val allFqNames = ArrayList<FqName?>()
        val packageClassNames = PackageIndexUtil.getAllPossiblePackageClasses(project).get(name)
        allFqNames.addAll(packageClassNames)

        val classOrObjectFqNames = JetClassShortNameIndex.getInstance().get(name, project, scope).map {
            it.fqName
        }
        allFqNames.addAll(classOrObjectFqNames)

        val fileFacadeFqNames = JetFileFacadeShortNameIndex.INSTANCE.get(name, project, scope).mapTo(LinkedHashSet()) {
            it.javaFileFacadeFqName
        }
        allFqNames.addAll(fileFacadeFqNames)
