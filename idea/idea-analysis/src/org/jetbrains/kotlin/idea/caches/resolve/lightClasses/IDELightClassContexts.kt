...
import org.jetbrains.kotlin.idea.caches.resolve.lightClasses.IDELightClassConstructionContext.Token.EXACT
import org.jetbrains.kotlin.idea.caches.resolve.lightClasses.IDELightClassConstructionContext.Token.LIGHT
...
class IDELightClassConstructionContext(bindingContext: BindingContext, module: ModuleDescriptor, val token: Token)
...
object IDELightClassContexts {
    ...
    private fun setupAdHocResolve(project: Project, realWorldModule: ModuleDescriptor, files: List<KtFile>): ResolveSession {
        ...
        val container = createContainer("LightClassStub", DefaultAnalyzerFacade.targetPlatform) {
            ...
            useInstance(LanguageSettingsProvider.getInstance(project).getLanguageVersionSettings(files.first().getModuleInfo(), project))
