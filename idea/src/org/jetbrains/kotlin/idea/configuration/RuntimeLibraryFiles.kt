...
class RuntimeLibraryFiles(
        val runtimeJar: File,
        val reflectJar: File?,
        val serializationRuntimeJar: File?,
        val runtimeSourcesJar: File
) {
    fun getAllJars(): List<File> = listOfNotNull(runtimeJar, reflectJar, serializationRuntimeJar, runtimeSourcesJar)
    ...
    fun getSerializationRuntimeDestination(dirToCopyJar: String): File? =
            if (serializationRuntimeJar != null) File(dirToCopyJar + "/" + serializationRuntimeJar.name) else null
