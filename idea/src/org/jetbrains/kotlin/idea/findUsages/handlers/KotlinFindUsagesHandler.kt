    ...
    override fun findReferencesToHighlight(target: PsiElement, searchScope: SearchScope): Collection<PsiReference> {
        ...
        searchReferences(target, object : Processor<UsageInfo> {
            override fun process(info: UsageInfo): Boolean {
                ...
                if (reference != null) {
                    var currentFile = reference.element.containingFile
                    val injectedLanguageManager = InjectedLanguageManager.getInstance(project)
                    if (injectedLanguageManager.isInjectedFragment(currentFile)) {
                        currentFile = injectedLanguageManager.getInjectionHost(currentFile)?.containingFile
                    }
                    if (scopeContainingFile != null && currentFile != scopeContainingFile) {
