...
object BranchedFoldingUtils {
    fun getFoldableBranchedAssignment(branch: KtExpression?): KtBinaryExpression? {
        ...
        return (branch?.lastBlockStatementOrThis() as? KtBinaryExpression)?.ensureOrNull(::checkAssignment)
    }

    fun getFoldableBranchedReturn(branch: KtExpression?): KtReturnExpression? {
        return (branch?.lastBlockStatementOrThis() as? KtReturnExpression)?.ensureOrNull { it.returnedExpression != null }
    }
