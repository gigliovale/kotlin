...
object BranchedFoldingUtils {
    fun getFoldableBranchedAssignment(branch: KtExpression?): KtBinaryExpression? {
        ...
        return (branch?.lastBlockStatementOrThis() as? KtBinaryExpression)?.keepIf(::checkAssignment)
    }

    fun getFoldableBranchedReturn(branch: KtExpression?): KtReturnExpression? {
        return (branch?.lastBlockStatementOrThis() as? KtReturnExpression)?.keepIf { it.returnedExpression != null }
    }
