...
object FindTransformationMatcher : TransformationMatcher {
    ...
    private fun KtExpression.isFilterForContainsOperation(inputVariable: KtCallableDeclaration, loop: KtForExpression): KtExpression? {
        ...
        return when {
            left.isVariableReference(inputVariable) -> right?.keepIf { it.isStableInLoop(loop, false) }
            right.isVariableReference(inputVariable) -> left?.keepIf { it.isStableInLoop(loop, false) }
            else -> null
        }
    }
}
