...
class CommaInWhenConditionWithoutArgumentFix(element: PsiElement) : KotlinQuickFixAction<PsiElement>(element) {
    override fun getFamilyName(): String = text
    override fun getText(): String = "Replace ',' with '||' in when-condition"

    override fun invoke(project: Project, editor: Editor?, file: KtFile) =
            replaceCommasWithOrs(element)

    companion object : KotlinSingleIntentionActionFactory() {
        override fun createAction(diagnostic: Diagnostic): IntentionAction? =
                CommaInWhenConditionWithoutArgumentFix(diagnostic.psiElement)
        ...
        private fun replaceCommasWithOrs(element: PsiElement) {
            val whenEntry = element.parent as? KtWhenEntry ?: return
            replaceCommasWithOrsInWhenEntry(whenEntry)
        }

        private fun replaceCommasWithOrsInWhenEntry(whenEntry: KtWhenEntry) {
