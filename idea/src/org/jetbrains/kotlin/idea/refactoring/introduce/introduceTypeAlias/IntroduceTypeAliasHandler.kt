...
object KotlinIntroduceTypeAliasHandler : RefactoringActionHandler {
    ...
    fun doInvoke(
        ...
        val errorMessage = when (elementToExtract) {
            is KtSimpleNameExpression -> if (!isTypeConstructorReference(elementToExtract)) "Expression must a type constructor reference" else null
            !is KtTypeElement -> "No type to refactor"
            else -> null
        }
        if (errorMessage != null) return showErrorHint(project, editor, "No type to refactor", REFACTORING_NAME)
