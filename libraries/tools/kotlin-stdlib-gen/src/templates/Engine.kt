    ...
    fun build(builder: StringBuilder, f: Family, primitive: PrimitiveType?) {
        ...
        fun effectiveTypeParams(): List<String> {
            ...
            if (f == Generic) {
                ...
                fun getImplicitTypeParameters(type: String) = type.dropWhile { it != '<' }.drop(1).takeWhile { it != '>' }.split(",")
                        .map { it.removePrefix("out").removePrefix("in").trim() }
                val implicitTypeParameters = getImplicitTypeParameters(receiver) + types.flatMap { getImplicitTypeParameters(it) }
